...
export class lametric_api_client {
    ...
    public sendNotification(text : String, category : String, icon : String, sound: String, cycles: Number, repeat: Number){
        ...
        if(sound != null && sound != '')
            data = JSON.stringify({
                priority: 'warning', model: { cycles: cycles, frames: [ { icon: icon, text : text } ],
                    sound: { category: category, id: sound, repeat: repeat }
                }
            });
